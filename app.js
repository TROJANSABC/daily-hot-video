const state = { data: null, weekly: null, platform: "all", query: "" };
const platformNames = { all: "全部平台", douyin: "抖音", kuaishou: "快手", xiaohongshu: "小红书", weekly: "AI 周报" };
...
const weeklyPanel = document.querySelector("#weeklyPanel");
...
// hot lists are keyword topics; their real video-frame covers are downloaded
// into ./data/covers/ at build time. If a cover is still missing we render a
// deterministic per-item SVG so every card has a picture.
...
function filteredItems() {
  ...
  return items.filter((item) => {
    ...
    const queryMatched = !keyword || `${item.title} ${item.summary || ""}`.toLowerCase().includes(keyword);
...
function renderList() {
  const isWeekly = state.platform === "weekly";
  boardEl.hidden = isWeekly;
  ...
  items.forEach((item, index) => {
    ...
    const sumEl = node.querySelector(".summary");
    if (item.summary) {
...
function renderWeekly() {
  ...
  if (!w || !w.report) {
    ...
    p.textContent = "周报正在生成中，敬请期待（每周一自动更新）。";
    weeklyBody.append(p);
    return;
  }
  weeklyMeta.textContent = `覆盖 ${w.days_covered || 0} 天 · ${formatTime(w.generated_at)}`;
  const lead = document.createElement("p");
  lead.className = "weekly-lead";
  lead.textContent = w.report;
  weeklyBody.append(lead);
  if (Array.isArray(w.top_topics) && w.top_topics.length) {
    const h = document.createElement("h3");
    h.textContent = "高频热点话题";
    weeklyBody.append(h);
    const ol = document.createElement("ol");
    ol.className = "weekly-topics";
    w.top_topics.forEach(([title, count]) => {
      const li = document.createElement("li");
      const s = document.createElement("span");
      s.textContent = title;
      const b = document.createElement("b");
      b.textContent = `${count} 次`;
      li.append(s, b);
      ol.append(li);
    });
    weeklyBody.append(ol);
  }
}
function render() {
...
async function loadData() {
  ...
  try {
    const weeklyRes = await fetch("./data/weekly.json", { cache: "no-store" });
